...
public class Relational_expression implements Expression
{
    Expression expression1;
    Expression expression2;
    String operation;

    public Relational_expression(String op, Expression e1, Expression e2)
    {
        this.expression1 = e1;
        this.expression2 = e2;
        this.operation = op;
    ...
    public Value evaluate(My_I_dictionary<String, Value> table, My_I_heap<Value> heap) throws MyException
    {
        ...
        if (value1.get_type().equals(new Int_type())) {
            ...
            if (value2.get_type().equals(new Int_type())) {
                ...
                if (operation.equals("<")) return new Bool_value(n1<n2);
                if (operation.equals("<=")) return new Bool_value(n1<=n2);
                if (operation.equals("==")) return new Bool_value(n1==n2);
                if (operation.equals("!=")) return new Bool_value(n1!=2);
                if (operation.equals(">")) return new Bool_value(n1>n2);
                if (operation.equals(">=")) return new Bool_value(n1>=n2);
            }
            else
